...
public class PanzerBattleOperator implements BattleOperator {
    ...
    public String battle(Vehicle attacker, Vehicle target) {
        ...
        while(!isSomeoneDead) {
            if(isAttackerTurn) {
                targetHitPoints -= Math.max(0, (long) Math.ceil(attackerAttack - (targetDefense + (targetWeight / 2))));
                isAttackerTurn = false;
            } else {
                attackerHitPoints -= Math.max(0,(long) Math.ceil(targetAttack - (attackerDefense + (attackerWeight / 2))));
                isAttackerTurn = true;
            }
